...
for trabajo in trabajos:
    particion_asignada = None  # Sirve para ver si fue posible agregar
    mejor_particion = None  # Para el Mejor Ajuste (Almacena toda la particion)

    # Encontrar la mejor partición disponible para el trabajo
    for i, particion in enumerate(particiones):
        if particion[0] == 'Libre' and (mejor_particion is None or particion[1] < mejor_particion[1]) and trabajo["tamano"] <= particion[1]:
            mejor_particion = particion
            particion_asignada = i
    #Asignaciones
    if particion_asignada is not None:
        particiones[particion_asignada][0] = trabajo["nombre"]
        particiones[particion_asignada][2] = trabajo['tamano']
        fragmentacion_interna[particion_asignada] = particiones[particion_asignada][1] - trabajo['tamano']
        print(f"El trabajo '{trabajo['nombre']}' se ha cargado en la partición {particion_asignada + 1}.")
    else:
        cola_trabajos_pendientes.append(trabajo)
        print("El trabajo se agregó a la cola de trabajos pendientes.")
...
for i, particion in enumerate(particiones):
    if cola_trabajos_pendientes and cola_trabajos_pendientes[0]['tamano'] <= particion[1]:
        ...
        fragmentacion_interna[i] = particiones[i][1] - trabajo_pendiente['tamano']
